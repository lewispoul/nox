...
Our goal is to simplify the project structure. We currently have three similar directories: `api/`, `nox_api/`, and `nox-api/`. 
Please consolidate them into a single package named `nox_api` by moving the relevant modules under that folder.
After moving, update import statements across the project to reflect the new package location.
Ensure existing business logic and tests remain intact.