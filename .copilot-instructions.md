...
The current, canonical API package structure uses the `api/` directory.
Do not perform any directory consolidation between `api/`, `nox_api/`, and `nox-api/` unless a specific task or design document explicitly requests it.
When editing code, preserve the existing `api/`-based layout and update imports only as needed for the immediate change.